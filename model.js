...
class Model {
    constructor(name="normal_neonate", port=3000, rt_stepsize = 1.0, dc_sample = 0.015, dc_interval = 1.0) {
        this.port = port
        this.name = name
    ...
    start_websocket_server() {
      ...
      wss.on("connection", (ws) => {
        ...
        console.log("API STATUS: websocket connection established on port: " + this.port);
        ...
        ws.on("message", (data) => {
          ...
          switch (mes.command) {
            ...
            case "stop":
              ...
              break;
            case "calculate":
              this.calculate(parseFloat(mes.payload))
              break;
            default:
        ...
        ws.on("close", () => {
          ...
          console.log("API STATUS: websocket connection closed on port: " + this.port);
        });
      });
        ...
        switch (mes.command) {
          case "status":
            ...
            if (mes.payload == "model initialized") {
              ...
              // set the realtime_stepsize
              this.set_rt_stepsize()

              // set the datacollectorsample interval
              this.set_dc_sample()
              
              // set the datacollector update_interval
